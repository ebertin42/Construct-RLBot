...
## 2026-07-22 -- Nexto ported and verified; Necto RETRACTED (wrong obs)

**Nexto (~GC1) is ported, golden-tested and usable.** EARLPerceiver in candle:
query/kv preprocess, 2 pre-norm cross-attention blocks over a fixed kv, and the
ControlsPredictorDot head (action embeddings precomputed from the 90-row table,
which is byte-identical to our own). Obs builder validated against the REAL
NextoObsBuilder (16 cases, blue+orange), net validated against real torch logits
(<2e-3, argmax exact).

    champion ck_000320471040 vs Nexto:  0W/0D/96L   win_share 0.0000

So the measured ladder is: Nexto (shutout) > Immortal (we win 0.0953) > us.
Immortal remains the only contestable teacher, which vindicates the running arm.

**Necto result RETRACTED.** An initial bench read 96W/0D/0L (1.0000) and it is
GARBAGE. Necto's observation is NOT Nexto's, despite both being q32/kv24:
  * entities are [ball, players.., boosts..] -- ball FIRST, players offset by 1
  * relative transform subtracts pos AND velocity (5:11), and does NOT rotate
    into the self heading (Nexto subtracts pos only, then rotates)
  * is_teammate/is_opponent keyed to BLUE, then columns swapped for orange
  * column 21 is a stateful per-pad RESPAWN TIMER / per-car DEMO TIMER, not a
    binary flag -- carried across frames

Feeding it Nexto's obs produced a bot driving on nonsense that we then "beat"
96-0. **Every net-level golden test still passed**, because those feed the same
(q, kv, mask) to both torch and candle -- they verify the NET, not the OBS. The
obs is only verified when the fixture is generated by the bot's OWN builder,
which was done for Nexto and Immortal but not for Necto.

`ForeignKind::Necto` now REFUSES at construction with an explanatory error rather
than silently running. Enabling it needs a real NectoObsBuilder including the
stateful timers.
